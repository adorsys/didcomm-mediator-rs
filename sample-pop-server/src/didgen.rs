...
pub fn didgen() -> Result<String, Box<dyn Error>> {
...
fn gen_diddoc(authentication_key: JWK, assertion_key: JWK) -> String {
    ...
    let did = convert::url_to_did_web_id(&public_domain) //
        .expect("Failed to derive did:web address");
    ...
    std::fs::write(format!("{DIDDOC_DIR}/did.json"), &did_json)
        .expect("Failed to persist JSON document");
